...
function getThisWeekOrders(orders) {
  ...
  // createdAt 是 timestamp 的格式
  const weekStart =  dayjs().startOf("week")
  const weekEnd = dayjs().endOf("week")
  return orders.filter((order) =>{
    const orderDate = dayjs.unix(order.createdAt)
    return orderDate.isBefore(weekEnd) && orderDate.isAfter(weekStart)
    }
  )
}
...
function validateOrderUser(data) {
  ...
  if (!telRegex.test(data.tel)){
    errors.push(`電話必須是 09 開頭的 0 位數字`)
  }
...
function validateCartQuantity(quantity) {
  ...
  if(!Number.isInteger(quantity)){
    return { isValid: false , error : `數量必須為正整數`}
  }
  if (quantity < 1 ) {
  ...
  return { isValid: true , error : `數量正確` }
}
...
1. JSON 解析：fetch 需要手動呼叫 response.json() 解析回應，
   axios 會自動解析 JSON，直接從 response.data 取得資料

2. _錯誤處理：fetch 只有在網路錯誤時才會 reject，HTTP 狀態碼 4xx/5xx 不會被視為錯誤；
   axios 會將 HTTP 狀態碼 4xx/5xx 視為錯誤並 reject
